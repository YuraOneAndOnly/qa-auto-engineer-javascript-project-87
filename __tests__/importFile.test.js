...
const exampleOfImport = '__fixtures__/importFileTests/correctJSONImport.json';
...
test('import JSON file with correct path', () => {
  const fileHexContent = readFileSync(exampleOfImport);
  const detectedEncoding = jschardet.detect(fileHexContent);
  const referenceResult = readFileSync(exampleOfImport, detectedEncoding.encoding);
